...
class GameMap:
	...
	def place_entities(self, room, entities):
		...
		max_equipment_items_per_room = from_dungeon_level([[2, 1]], self.dungeon_level)
		...
		monster_chances = {
			...
			'Balrog': from_dungeon_level([[20, 7], [30, 9], [80, 11]], self.dungeon_level)
		}


		#Terrium, Ferrium, Aurium
		item_chances = {
			'healing_potion': from_dungeon_level([[25, 1], [10, 8]], self.dungeon_level),
			...
			'fireball_spell': from_dungeon_level([[25, 5]], self.dungeon_level),
			'confusion_spell': from_dungeon_level([[10, 2]], self.dungeon_level)
		}

		equipment_item_chances = {
			'equipment_health_potion': from_dungeon_level([[90, 1]], self.dungeon_level)
		}
		...
		for i in range(number_of_monsters):
			...
			if not any([entity for entity in entities if entity.x == x and entity.y == y]):
				...
				if monster_choice == 'goblin':
					fighter_component = Fighter(hp=5, defense=0, power=2, magic=0, xp=10)
					ai_component = BasicMonster()
					monster = Entity(x, y, 'g', libtcod.darker_chartreuse, 'Goblin', blocks=True,
						render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
				elif monster_choice == 'orc':
					fighter_component = Fighter(hp=12, defense=0, power=4, magic=0, xp=35)
					ai_component = BasicMonster()
					monster = Entity(x, y, 'o', libtcod.desaturated_green, 'Orc', blocks=True,
						render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
				elif monster_choice == 'troll':
					fighter_component = Fighter(hp=18, defense=2, power=8, magic=0, xp=100)
					ai_component = BasicMonster()
					monster = Entity(x, y, 'T', libtcod.darker_green, 'Troll', blocks=True, fighter=fighter_component,
						render_order=RenderOrder.ACTOR, ai=ai_component) 
				else:
					fighter_component = Fighter(hp=60, defense=5, power=16, magic=0, xp=200)
					ai_component = BasicMonster()
					monster = Entity(x, y, 'B', libtcod.darker_red, 'Balrog', blocks=True, fighter=fighter_component,
						render_order=RenderOrder.ACTOR, ai=ai_component)
